...
export class ChatroomService {
  ...
  private async sendAOMessage(processId: string, action: string, data?: any, tags?: Record<string, string>) {
    ...
    try {
      ...
      console.log(`Sending AO message to ${processId}:`, { action, data, tags: aoTags });

      // Use the AO SDK to send a message
    ...
    switch (action) {
      case 'GetInfo':
        return {
          Action: 'ChatroomInfo',
          Data: JSON.stringify({
            accessPrice: isUserProcess ? 1000 : Math.floor(Math.random() * 100) + 10, // Use actual AccessPrice from your Lua
      ...
      case 'ListData':
        ...
        if (isUserProcess) {
          ...
          const sources = ['uniswap_v3', 'compound', 'technical_analysis', 'social_feed'];
  ...
  // Join a chatroom
  async joinChatroom(processId: string): Promise<{ success: boolean; message: string; paymentRequired?: boolean }> {
    try {
  ...
  async postData(processId: string, data: string, dataType: string, source: string): Promise<{ success: boolean; message: string }> {
    try {
      const dataId = `data_${Date.now()}_${Math.random().toString(36).substr(2, 8)}`;
  ...
  async enhanceChatroomsWithRealData(mockChatrooms: any[]): Promise<ExtendedChatroom[]> {
    const enhanced: ExtendedChatroom[] = [];
    
    for (const mockRoom of mockChatrooms) {
      const processId = MOCK_PROCESS_IDS[mockRoom.id as keyof typeof MOCK_PROCESS_IDS];
      ...
      if (processId) {
        ...
        enhanced.push({
          ...
          description: realInfo ? `🔴 LIVE: ${realInfo.memberCount} active members, ${realInfo.dataCount} data entries` : mockRoom.description,
          // Dynamic transaction volume based on data activity
          transactionVolume: realInfo ? `$${(realInfo.dataCount * 125 + realInfo.memberCount * 50).toLocaleString()}` : mockRoom.transactionVolume,
          // Update dataset info to reflect real AO data
          dataset: realInfo ? `Live AO process data: ${realInfo.dataCount} messages from ${realInfo.memberCount} members` : mockRoom.dataset,
          // Add enhanced title for real processes
          title: processId === 'vyd3NOTV75D3ZEJ1bEpmbAKDuZ56GwnfeTsesK2uUtY' ? 
            `🎯 ${mockRoom.title}` : 
            realInfo ? `🔴 ${mockRoom.title}` : mockRoom.title
        });
        ...
        if (processId === 'vyd3NOTV75D3ZEJ1bEpmbAKDuZ56GwnfeTsesK2uUtY') {
          ...
          console.log(`💰 Access Price: ${realInfo?.accessPrice || 'unknown'} tokens`);
